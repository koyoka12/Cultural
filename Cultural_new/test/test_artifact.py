...
class TestTourQueue(unittest.TestCase):
    ...
    def test_add_group(self):
        group1 = VisitorGroup(1, datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "HIGH")
        group2 = VisitorGroup(2, (datetime.now() + timedelta(seconds=10)).strftime('%Y-%m-%d %H:%M:%S'), "LOW")
    ...
    def test_reschedule_group(self):
        ...
        self.queue.add_group(group1)
        self.queue.add_group(group2)
        self.queue.reschedule_group(1, (datetime.now() + timedelta(seconds=20)).strftime('%Y-%m-%d %H:%M:%S'), "LOW")
