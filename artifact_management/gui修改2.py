...
def main():
    ...
    btn_add_artifact = ttk.Button(btn_frame, text="Add Artifact", command=lambda: add_artifact(root, manager))
    btn_add_artifact.pack(side=tk.LEFT, padx=5, pady=5)
    btn_add_visitor = ttk.Button(btn_frame, text="Add Visitor Group", command=lambda: add_visitor_group(root, manager))
    ...
    btn_clear_csv = ttk.Button(btn_frame, text="Clear CSV", command=lambda: clear_csv(root, manager))
...
def add_artifact(root, manager):
    ask_for_artifact_info(root, manager)

def add_visitor_group(root, manager):
...
def save_to_csv(manager):
    # 将文物和游客组数据保存到 CSV 文件
    artifacts_df = pd.DataFrame([vars(a) for a in manager.artifacts])
    visitors_df = pd.DataFrame([vars(v) for v in manager.visitor_queue])
    artifacts_df.to_csv('artifacts.csv', index=False)
    visitors_df.to_csv('visitors.csv', index=False)
    print("Data saved to CSV.")

def clear_csv(root, manager):
    ...
    if artifact_id is not None:
        ...
        save_to_csv(manager)  # 假设管理器有此方法来保存更新后的数据到 CSV

def show_visitor_order(manager):
    # 显示游客进场顺序
    for index, group in enumerate(manager.visitor_queue, start=1):
        print(f"{index}. {group}")
